      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {Object.entries(dict.footer.productLinks).map(([key, label]) => (
                <li key={key}>
                  <Link href="#" className="text-sm hover:text-mizan-gold transition-colors font-arabic">
                    {label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {Object.entries(dict.footer.companyLinks).map(([key, label]) => (
                <li key={key}>
                  <Link href="#" className="text-sm hover:text-mizan-gold transition-colors font-arabic">
                    {label}
                  </Link>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {Object.entries(dict.footer.legalLinks).map(([key, label]) => (
                <i key={key}>
                  <li>
                    <Link href="#" className="text-sm hover:text-mizan-gold transition-colors font-arabic">
                      {label}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            {/* Social icons */}
            <a href="#" className="text-gray-500 hover:text-mizan-gold transition-colors">
